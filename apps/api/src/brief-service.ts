...
type BriefContent = { title:string; reflection:string; taskSummary:string; sections:BriefSection[] };
...
export class BriefService {
  ...
  async generateFromReview(reviewSessionId: string) {
    ...
    const [finance, ai, technology, humanities, taskExpansion, weather] = await Promise.all([
      ...
      taskQuery ? this.providers.search(taskQuery) : Promise.resolve({ results: [], source: null }), this.providers.weather()
    ]);
    ...
    const content: BriefContent = {
      title: `${review.localDate} 的每日简报`, reflection: messages.map((message) => message.content).join("\n\n"), taskSummary: `当天共安排 ${taskRows.length} 项任务，已关闭 ${closed} 项。`, sections: [...sections.map((item) => item.section), weather.section]
    };
