...
export class GrowthService {
  ...
  async getSummary(endLocalDate: string) {
    const dates = datesEndingAt(endLocalDate, 7);
    const start = dates[0]!;
    ...
    return {
      ...
      radar: [
        ...
        { key: "consistency", label: "连续", value: Math.round(activeDays / 7 * 100) },
        { key: "depth", label: "深度", value: Math.min(100, Math.round(deepFocusMinutes / 180 * 100)) },
        { key: "review", label: "复盘", value: Math.round(reviewedDays / 7 * 100) },
