...
import { z } from "zod";
import { LongRangeTaskTreeService, TaskTreeCandidateConflictError, TaskTreeCandidateNotFoundError, TaskTreeGenerationUnavailableError, TaskTreePlanNotFoundError, TaskTreeVersionConflictError } from "./long-range-task-tree-service.js";
...
function treeError(reply: FastifyReply, error: unknown) {
  if (error instanceof TaskTreePlanNotFoundError) return reply.status(404).send({ error: "long_range_plan_not_found" });
  if (error instanceof TaskTreeCandidateNotFoundError) return reply.status(404).send({ error: "task_tree_candidate_not_found" });
  if (error instanceof TaskTreeVersionConflictError) return reply.status(409).send({ error: "long_range_plan_version_conflict", plan: error.plan });
  if (error instanceof TaskTreeCandidateConflictError) return reply.status(409).send({ error: "task_tree_candidate_conflict", message: error.message });
  if (error instanceof TaskTreeGenerationUnavailableError) return reply.status(503).send({ error: "task_tree_generation_unavailable" });
  throw error;
}
