...
type Context = {
  ...
  focusSessions: Array<{ id: string; taskId: string; rawActiveSeconds: number; effectiveFocusSeconds: number; state: string }>;
...
export function ReviewWorkspace({ isWorkspaceCurrent = true }: { isWorkspaceCurrent?: boolean }) {
  const [review, setReview] = useState<Review | null>(null); const [messages, setMessages] = useState<Message[]>([]); const [radarSnapshot, setRadarSnapshot] = useState<RadarSnapshot | null>(null); const [context, setContext] = useState<Context | null>(null); const [weatherBrief, setWeatherBrief] = useState<WeatherBrief | null>(null); const [locationName, setLocationName] = useState(""); const [recentGrowthDays, setRecentGrowthDays] = useState<GrowthDay[]>([]); const [draft, setDraft] = useState(""); const [saving, setSaving] = useState(false); const [locating, setLocating] = useState(false); const [error, setError] = useState<string | null>(null); const [conversationOpen, setConversationOpen] = useState(false); const reviewInputRef = useRef<HTMLTextAreaElement>(null);
  const load = useCallback(async () => { const growthRequest = fetch(`${API}/api/v1/growth/summary?endDate=${localDate()}&days=30`).then(async (response) => response.ok ? (await response.json() as { summary: { days: GrowthDay[] } }).summary.days : []).catch(() => [] as GrowthDay[]); const data = await request<{ session: Review; messages: Message[]; radarSnapshot: RadarSnapshot | null; context: Context; briefs: WeatherBrief[] }>(`/api/v1/reviews/${localDate()}`); setReview(data.session); setMessages(data.messages); setRadarSnapshot(data.radarSnapshot); setContext(data.context); setRecentGrowthDays(await growthRequest); let latest = [...data.briefs].reverse().find((brief) => brief.content.title === "今日地点及天气情况") ?? null; const savedLocation = latest?.content.location; const legacyCoordinates = locationCoordinates(savedLocation?.name); if (latest && savedLocation && legacyCoordinates) { try { latest = (await request<{ brief: WeatherBrief }>(`/api/v1/reviews/${data.session.id}/location-weather`, "POST", { latitude: savedLocation.latitude, longitude: savedLocation.longitude })).brief; } catch { latest = null; } } setWeatherBrief(latest); const name = latest?.content.location?.name; setLocationName(name && !locationCoordinates(name) ? name : ""); }, []);
  useEffect(() => { void load().catch(() => setError("无法加载今日复盘，请确认 API 正在运行。")); }, [load]);
  const plannedTaskCount = useMemo(() => context?.tasks.filter((task) => task.recordKind !== "backfill" && task.lifecycleStatus !== "cancelled").length ?? 0, [context]); const effective = useMemo(() => Math.round((context?.focusSessions.reduce((sum, session) => sum + session.effectiveFocusSeconds, 0) ?? 0) / 60), [context]); const completed = useMemo(() => context?.tasks.filter((task) => task.currentOutcome === "complete").length ?? 0, [context]); const userMessages = useMemo(() => messages.filter((message) => message.source === "app"), [messages]); const conversationMessages = useMemo(() => messages.filter((message) => message.source === "app" || message.source === "ai"), [messages]); const lastConversationMessage = conversationMessages.at(-1);
  const streakDays = useMemo(() => { const currentDate = review?.localDate ?? localDate(); const days = [...recentGrowthDays].sort((left, right) => left.localDate.localeCompare(right.localDate)); let streak = 0; for (let index = days.length - 1; index >= 0; index -= 1) { const day = days[index]!; const currentReviewActivity = day.localDate === currentDate && userMessages.length > 0; if (!(day.focusMinutes > 0 || day.closedTasks > 0 || day.points > 0 || currentReviewActivity)) break; streak += 1; } return streak; }, [recentGrowthDays, review?.localDate, userMessages.length]);
  const taskSummaries = useMemo(() => {
    ...
    return context.tasks.flatMap((task) => {
      ...
      return [{ ...task, outcome, feedback, focusSeconds }];
    });
  }, [context]);
  ...
  return <section className="page review-page" aria-labelledby="review-title">
    ...
    <section className="review-colophon" aria-label="今日题记"><div className="review-colophon-heading"><p className="section-kicker">今日回看</p><time dateTime={review?.localDate ?? localDate()}>{reviewDateLabel(review?.localDate ?? localDate())}</time></div><div className="review-completion-mark"><strong>{chineseNumber(completed)}</strong><span>今日完成</span></div><dl className="review-colophon-metrics"><div><dt>计划</dt><dd>{plannedTaskCount}<small>项</small></dd></div><div><dt>专注</dt><dd>{effective}m</dd></div></dl><div className="review-growth-plant" data-growth-stage={effective >= 60 ? "two-leaf" : effective >= 20 ? "sprout" : "seed"} data-reviewed={userMessages.length > 0 ? "true" : "false"} style={{ "--review-streak-growth": `${Math.min(streakDays, 7) * 6}px` } as CSSProperties} aria-label={`今日植物：有效专注 ${effective} 分钟`}><i className="review-seed" /><i className="review-stem" /><i className="review-leaf leaf-one" /><i className="review-leaf leaf-two" /><i className="review-new-leaf" /></div></section>
    ...
    <section className="review-task-summary" aria-labelledby="review-task-summary-title"><div className="review-section-heading"><div><p className="section-kicker">任务复盘</p><h2 id="review-task-summary-title">今天发生了什么</h2></div><span>{taskSummaries.length} 项任务</span></div>{taskSummaries.length ? <div className="review-task-summary-list">{taskSummaries.map((task) => <article key={task.id}><div><strong>{task.title}</strong><small>{timeLabel(task.startAt)} – {timeLabel(task.endAt)}</small></div><p><b>做了什么：</b>{task.title}<br /><b>完成情况：</b>{outcomeLabel(task.outcome?.outcome ?? task.currentOutcome, task.outcome?.progressPercent)} · 有效专注 {Math.round(task.focusSeconds / 60)} 分钟<br /><b>最后反馈：</b>{satisfactionLabel(task.feedback?.satisfaction)}</p></article>)}</div> : <p className="review-radar-empty">今天还没有可复盘的正式任务。</p>}</section>
