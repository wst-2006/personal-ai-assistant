...
export function ReviewWorkspace({ isWorkspaceCurrent = true }: { isWorkspaceCurrent?: boolean }) {
  ...
  useEffect(() => {
    ...
    setDraft(""); setError(null); setConversationOpen(false); setTaskSummaryExpanded(false);
  ...
  return <section className="page review-page" aria-labelledby="review-title">
    ...
    <div className="review-board">
      ...
      <aside className="review-side-column" aria-label={viewingToday ? "今日侧边信息" : "历史侧边信息"}>
        ...
        <section className="review-location-panel" aria-labelledby="review-location-title"><div className="review-section-heading"><div><p className="section-kicker">地点及天气情况</p><h2 id="review-location-title">{viewingToday ? "需要时记录一次。" : "历史记录"}</h2></div><MapPin /></div><div className="review-location-controls"><label><span>所在城市或行政区</span><input aria-label="所在城市或行政区" disabled={!canEdit || !review || saving} value={locationName} onChange={(event) => setLocationName(event.target.value)} placeholder="例如：昆明" maxLength={120} /></label><div className="review-location-buttons"><button className="quiet-button" type="button" disabled={!canEdit || !review || saving || !locationName.trim()} onClick={() => void recordWeather({ locationName: locationName.trim() })}><MapPin />记录手动地点</button><button className="quiet-button" type="button" disabled={!canEdit || !review || saving || locating} onClick={() => void useDeviceLocation()}><LocateFixed />{locating ? "正在读取" : "读取本机位置"}</button></div></div>{weatherBrief?.content.location && !locationCoordinates(weatherBrief.content.location.name) && (() => { const weather = weatherBrief.content.weather; const presentation = weatherPresentation(weather?.weatherCode); const WeatherIcon = presentation.Icon; return <div className="review-weather-result"><div className="review-weather-main"><WeatherIcon aria-hidden="true" /><div><strong>{weather ? `${weather.temperatureCelsius}°` : "--"}</strong><span>{presentation.label}</span></div></div><div className="review-weather-meta"><time>{weatherDateLabel(weather?.observedAt)}</time><p>{weatherBrief.content.location.name}</p><small>{weather ? `体感 ${weather.apparentTemperatureCelsius}°C` : "天气暂不可用"}</small></div></div>; })()}</section>
      </aside>
    </div>
    <section className="review-task-summary" aria-labelledby="review-task-summary-title"><div className="review-section-heading"><div><p className="section-kicker">任务复盘</p><h2 id="review-task-summary-title">{viewingToday ? "今天发生了什么" : "这一天发生了什么"}</h2></div><span>{taskSummaries.length} 项任务</span></div>{taskSummaries.length ? <><div className={`review-task-summary-list ${taskSummaryExpanded ? "expanded" : ""}`}>{displayedTaskSummaries.map((task) => <article key={task.id}><div><strong>{task.title}</strong><small>{timeLabel(task.startAt)} – {timeLabel(task.endAt)}</small></div><p><b>做了什么：</b>{task.title}<br /><b>完成情况：</b>{outcomeLabel(task.outcome?.outcome ?? task.currentOutcome ?? (task.startedWithinWindow ? "complete" : null), task.outcome?.progressPercent)} · 有效专注 {Math.round(task.focusSeconds / 60)} 分钟<br /><b>最后反馈：</b>{satisfactionLabel(task.feedback?.satisfaction)}</p></article>)}</div>{taskSummaries.length > 2 && <button className="review-task-summary-toggle" type="button" aria-expanded={taskSummaryExpanded} onClick={() => setTaskSummaryExpanded((value) => !value)}>{taskSummaryExpanded ? "收起任务" : "展开查看最多 4 项"}<ChevronDown className={taskSummaryExpanded ? "open" : ""} /></button>}</> : <p className="review-radar-empty">{viewingToday ? "今天还没有可复盘的正式任务。" : "这一天还没有可复盘的正式任务。"}</p>}</section>
