...
import { Check, ChevronDown, ChevronLeft, ChevronRight, Cloud, CloudFog, CloudLightning, CloudRain, CloudSnow, CloudSun, LocateFixed, MapPin, MessageCircle, RefreshCw, Sun } from "lucide-react";
...
function isIsoDate(value: string) { return /^\d{4}-\d{2}-\d{2}$/u.test(value); }
function shiftDate(value: string, days: number) {
  const [year, month, day] = value.split("-").map(Number);
  return new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Shanghai", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(Date.UTC(year, month - 1, day + days, 12)));
}
...
export function ReviewWorkspace({ isWorkspaceCurrent = true }: { isWorkspaceCurrent?: boolean }) {
  ...
  useEffect(() => {
    ...
    setSelectedDate((current) => current === previous ? todayDate : current);
  }, [todayDate]);
  const load = useCallback(async (date: string) => {
    const requestId = ++loadRequestRef.current;
    const isCurrent = () => requestId === loadRequestRef.current;
    const data = await request<{ session: Review; messages: Message[]; radarSnapshot: RadarSnapshot | null; context: Context; briefs: WeatherBrief[] }>(`/api/v1/reviews/${date}`);
    if (!isCurrent()) return;
    setReview(data.session); setMessages(data.messages); setRadarSnapshot(data.radarSnapshot); setContext(data.context);
    let latest = [...data.briefs].reverse().find((brief) => brief.content.title === "今日地点及天气情况") ?? null;
    const savedLocation = latest?.content.location; const legacyCoordinates = locationCoordinates(savedLocation?.name);
    if (date === localDate() && latest && savedLocation && legacyCoordinates) {
      try { latest = (await request<{ brief: WeatherBrief }>(`/api/v1/reviews/${data.session.id}/location-weather`, "POST", { latitude: savedLocation.latitude, longitude: savedLocation.longitude })).brief; }
      catch { latest = null; }
      if (!isCurrent()) return;
    }
    setWeatherBrief(latest); const name = latest?.content.location?.name; setLocationName(name && !locationCoordinates(name) ? name : "");
  }, []);
  useEffect(() => {
    if (!isIsoDate(selectedDate)) return;
    setDraft(""); setError(null); setConversationOpen(false);
    setReview(null); setMessages([]); setRadarSnapshot(null); setContext(null); setWeatherBrief(null); setLocationName("");
    const pending = load(selectedDate);
    const requestId = loadRequestRef.current;
    void pending.catch(() => { if (requestId === loadRequestRef.current) setError("无法加载这一天的复盘，请确认本机服务正在运行。"); });
  }, [load, selectedDate]);
  const viewingToday = selectedDate === todayDate;
  const canEdit = isWorkspaceCurrent && viewingToday;
  ...
  async function save() { if (!review || !draft.trim() || !canEdit) return; setSaving(true); setError(null); try { const result = await request<{ session: Review; message: Message }>(`/api/v1/reviews/${review.id}/messages`, "POST", { content: draft.trim(), source: "app" }); setReview(result.session); setMessages((current) => [...current, result.message]); setDraft(""); } catch { setError("这段复盘没有保存，请重试。"); } finally { setSaving(false); } }
  async function replyToLast() { if (!review || lastConversationMessage?.source !== "app" || !canEdit) return; setSaving(true); setError(null); try { const result = await request<{ session: Review; messages: Message[] }>(`/api/v1/reviews/${review.id}/reply-last`, "POST"); setReview(result.session); setMessages(result.messages); } catch { setError("最近一条复盘仍然保留，AI 暂时没有回复；请稍后重试。"); await load(selectedDate).catch(() => undefined); } finally { setSaving(false); } }
  async function recordWeather(body: Record<string, unknown>) { if (!review || !canEdit) return; setSaving(true); setError(null); try { const result = await request<{ brief: WeatherBrief }>(`/api/v1/reviews/${review.id}/location-weather`, "POST", body); const name = result.brief.content.location?.name; if (locationCoordinates(name)) { setWeatherBrief(null); setLocationName(""); setError("本机位置已读取，但城市识别没有返回结果。请重启 API 后再读取一次位置。"); } else { setWeatherBrief(result.brief); if (name) setLocationName(name); } } catch { setError("地点或天气没有记录，请检查地点后重试。"); } finally { setSaving(false); } }
  ...
  const resolvedWeather = weatherBrief?.content.location && !locationCoordinates(weatherBrief.content.location.name)
    ? weatherBrief
    : null;
  return <section className="page review-page" aria-labelledby="review-title">
    <DecorativePlant kind="plum" note="疏影留白" accent="frost" className="review-page-plant" />
    <div className="review-heading"><div><p className="eyebrow">{viewingToday ? "一天将要落幕" : "历史复盘"}</p><h1 id="review-title">{viewingToday ? "把今天还给自己。" : "把这一天留给自己。"}</h1><div className="review-date-controls"><button className="quiet-button review-date-step" type="button" aria-label="前一天" title="前一天" onClick={() => setSelectedDate(shiftDate(selectedDate, -1))}><ChevronLeft /></button><label><span>查看日期</span><input aria-label="复盘日期" type="date" max={todayDate} value={selectedDate} onChange={(event) => { const nextDate = event.target.value; if (isIsoDate(nextDate)) setSelectedDate(nextDate); }} /></label><button className="quiet-button review-date-step" type="button" aria-label="后一天" title="后一天" disabled={selectedDate >= todayDate} onClick={() => setSelectedDate(shiftDate(selectedDate, 1))}><ChevronRight /></button>{!viewingToday && <button className="quiet-button" type="button" onClick={() => setSelectedDate(todayDate)}>回到今天</button>}</div></div><div className="review-count"><strong>{userMessages.length}</strong><span>条我的复盘</span></div></div>
    <section className="review-colophon" aria-label="复盘题记"><div className="review-colophon-heading"><p className="section-kicker">{viewingToday ? "今日回看" : "历史回看"}</p><time dateTime={review?.localDate ?? selectedDate}>{reviewDateLabel(review?.localDate ?? selectedDate)}</time></div><div className="review-completion-mark"><strong>{chineseNumber(completed)}</strong><span>完成</span></div><dl className="review-colophon-metrics"><div><dt>计划</dt><dd>{plannedTaskCount}<small>项</small></dd></div><div><dt>专注</dt><dd>{effective}m</dd></div></dl></section>
    <div className="review-board">
      <div className="review-main-column">
        <section className="review-sheet" aria-labelledby="review-fragment-title"><header><div><p className="section-kicker">每日复盘</p><h2 id="review-fragment-title">留下这一天的感受。</h2></div><span>{userMessages.length ? `已留下 ${userMessages.length} 笔` : "纸面尚空"}</span></header><div className="review-writing-field" aria-live="polite">{conversationMessages.length === 0 ? <div className="stream-empty"><MessageCircle /><p>第一句话，会在这里亮起。</p></div> : <div className="review-fragment-list">{conversationMessages.map((message, index) => <article className={message.source === "ai" ? "ai" : "app"} key={message.id}><span>{message.source === "ai" ? "AI 回应" : `第 ${chineseNumber(index + 1)} 笔`}</span><p>{message.content}</p></article>)}</div>}<textarea ref={reviewInputRef} aria-label="每日复盘" disabled={!canEdit || !review} value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="今天下午忽然觉得……" rows={userMessages.length ? 1 : 6} maxLength={2000} /></div><footer className="review-sheet-footer"><span>{draft.length}/2000</span><div className="review-composer-actions"><button className="primary-button review-save-button" disabled={!canEdit || !draft.trim() || saving} onClick={() => void save()}><Check />保存</button></div></footer>{lastConversationMessage?.source === "app" && <button className="text-button review-reply-last" type="button" disabled={!canEdit || saving} onClick={() => void replyToLast()}><RefreshCw />{saving ? "正在等待 AI" : "请 AI 回应最近一条"}</button>}
      ...
      <aside className="review-side-column" aria-label={viewingToday ? "今日侧边信息" : "历史侧边信息"}>
        <section className="review-radar-archive" aria-labelledby="review-radar-title"><div className="review-radar-archive-heading"><div><p className="section-kicker">{viewingToday ? "今日六维" : "历史六维"}</p><h2 id="review-radar-title">把体验留成一张图。</h2></div><span>{radarSnapshot ? "已保存" : "尚未保存"}</span></div>{radarSnapshot ? <div className="review-radar-snapshot"><RadarChart compact values={radarSnapshot.radar} ariaLabel={viewingToday ? "今日六维回看快照" : "历史六维回看快照"} /><div className="review-radar-snapshot-copy"><strong>{viewingToday ? "今日体验快照" : "这一天的体验快照"}</strong><p>这张图来自你最后一次确认的六维评分，会固定保留在这一天的复盘里。</p></div></div> : <p className="review-radar-empty">{viewingToday ? "今天还没有保存六维体验。" : "这一天还没有保存六维体验。"}</p>}</section>
        <section className="review-location-panel" aria-labelledby="review-location-title"><div className="review-section-heading"><div><p className="section-kicker">地点及天气情况</p><h2 id="review-location-title">{viewingToday ? "需要时记录一次。" : "历史记录"}</h2></div><MapPin /></div><div className="review-location-controls"><label><span>所在城市或行政区</span><input aria-label="所在城市或行政区" disabled={!canEdit || !review || saving} value={locationName} onChange={(event) => setLocationName(event.target.value)} placeholder="例如：输入城市或行政区名称" maxLength={120} /></label><div className="review-location-buttons"><button className="quiet-button" type="button" disabled={!canEdit || !review || saving || !locationName.trim()} onClick={() => void recordWeather({ locationName: locationName.trim() })}><MapPin />记录手动地点</button><button className="quiet-button" type="button" disabled={!canEdit || !review || saving || locating} onClick={() => void useDeviceLocation()}><LocateFixed />{locating ? "正在读取" : "读取本机位置"}</button></div></div><div className={`review-weather-result ${resolvedWeather ? "has-data" : "empty"}`}>{resolvedWeather ? (() => { const weather = resolvedWeather.content.weather; const presentation = weatherPresentation(weather?.weatherCode); const WeatherIcon = presentation.Icon; return <><div className="review-weather-main"><WeatherIcon aria-hidden="true" /><div><strong>{weather ? `${weather.temperatureCelsius}°` : "--"}</strong><span>{presentation.label}</span></div></div><div className="review-weather-meta"><time>{weatherDateLabel(weather?.observedAt)}</time><p>{resolvedWeather.content.location!.name}</p><small>{weather ? `体感 ${weather.apparentTemperatureCelsius}°C` : "天气暂不可用"}</small></div></>; })() : null}</div></section>
      </aside>
    </div>
    <section className="review-task-summary" aria-labelledby="review-task-summary-title"><div className="review-section-heading"><div><p className="section-kicker">任务复盘</p><h2 id="review-task-summary-title">{viewingToday ? "今天发生了什么" : "这一天发生了什么"}</h2></div><span>{taskSummaries.length} 项任务</span></div>{taskSummaries.length ? <div className="review-task-summary-list">{taskSummaries.map((task) => <article key={task.id}><div><strong>{task.title}</strong><small>{timeLabel(task.startAt)} – {timeLabel(task.endAt)}</small></div><p><b>做了什么：</b>{task.title}<br /><b>完成情况：</b>{outcomeLabel(task.outcome?.outcome ?? task.currentOutcome ?? (task.startedWithinWindow ? "complete" : null), task.outcome?.progressPercent)} · 有效专注 {Math.round(task.focusSeconds / 60)} 分钟<br /><b>最后反馈：</b>{satisfactionLabel(task.feedback?.satisfaction)}</p></article>)}</div> : <p className="review-radar-empty">{viewingToday ? "今天还没有可复盘的正式任务。" : "这一天还没有可复盘的正式任务。"}</p>}</section>
