...
export function TodayWorkspace({ onFocus, onOpenHealth, healthTaskDraft, onHealthTaskDraftConsumed, refreshToken=0 }:{ onFocus:(id:string)=>void; onOpenHealth:()=>void; healthTaskDraft?:HealthTaskDraft|null; onHealthTaskDraftConsumed?:()=>void; refreshToken?:number }) {
  ...
  function actionMenu(task:Task){if(actionTaskId!==task.id||task.lifecycleStatus==="active")return null;return <div className="task-action-menu" onPointerDown={e=>e.stopPropagation()}>{task.lifecycleStatus==="open"&&<button onClick={()=>{setActionTaskId(null);editTask(task);}}><Pencil/>编辑任务</button>}{task.lifecycleStatus==="open"&&<button onClick={()=>{setActionTaskId(null);onFocus(task.id);}}><Play/>开始专注</button>}{(task.lifecycleStatus==="open"||task.lifecycleStatus==="awaiting_outcome")&&<button onClick={()=>openOutcome(task)}><CheckCircle2/>记录结果</button>}<button onClick={()=>void openOutcomeHistory(task)}><History/>结果历史</button>{task.lifecycleStatus==="open"&&<button onClick={()=>void runTaskAction(task,"cancel")}><XCircle/>取消任务</button>}{task.lifecycleStatus==="open"&&<button className="danger" onClick={()=>void runTaskAction(task,"delete")}><Trash2/>删除任务</button>}{(task.lifecycleStatus==="closed"||task.lifecycleStatus==="cancelled")&&<button onClick={()=>void runTaskAction(task,"reopen")}><RotateCcw/>重新打开</button>}</div>;}
