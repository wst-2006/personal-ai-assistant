...
import { AlertTriangle, CalendarPlus, Check, CheckCircle2, CircleDashed, GripHorizontal, History, Lightbulb, LoaderCircle, MoreHorizontal, Pencil, Play, RotateCcw, Sparkles, Trash2, X, XCircle } from "lucide-react";
...
type FormState = { entryMode:"schedule"|"backfill"; title:string; scheduleKind:ScheduleKind; localDate:string; daypart:Daypart; start:string; end:string; notes:string };
...
const emptyForm = (date=today(), start?:string, end?:string, entryMode:FormState["entryMode"]="schedule"):FormState => {
  ...
  return { entryMode, title:"", scheduleKind:"exact", localDate:date, daypart:"morning", start:start ?? hhmm(startMinute), end:end ?? hhmm(Math.min(LAST_TIMELINE_MINUTE,startMinute+TIMELINE_STEP_MINUTES)), notes:"" };
};
...
export function TodayWorkspace({ onFocus, refreshToken=0 }:{ onFocus:(id:string)=>void; refreshToken?:number }) {
  ...
  function editTask(task:Task){setEditing(task);setSource(null);setForm({entryMode:"schedule",title:task.title,scheduleKind:task.scheduleKind,localDate:task.localDate??date,daypart:task.daypart??"morning",start:hhmm(minuteOf(task.startAt)),end:hhmm(minuteOf(task.endAt)),notes:task.notes??""});}
