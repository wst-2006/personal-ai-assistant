...
const HOUR_PX = 72;
const DAY_PX = HOUR_PX * ((TIMELINE_END_MINUTE-TIMELINE_START_MINUTE)/60);
...
const snapTimelineMinute = (minute:number) => Math.max(TIMELINE_START_MINUTE, Math.min(LAST_TIMELINE_MINUTE, Math.round(minute/TIMELINE_STEP_MINUTES)*TIMELINE_STEP_MINUTES));
const isTimelineMinute = (minute:number) => minute % TIMELINE_STEP_MINUTES === 0;
const timelineTop = (minute:number) => (minute-TIMELINE_START_MINUTE)/60*HOUR_PX;
...
const normalizedTimelineRange = (anchor:number,current:number) => {
  const low=Math.max(TIMELINE_START_MINUTE,Math.min(anchor,current));const high=Math.min(TIMELINE_END_MINUTE,Math.max(anchor,current));
...
export function TodayWorkspace({ onFocus, onOpenHealth, healthTaskDraft, onHealthTaskDraftConsumed, planChangeTaskEditRequest, onPlanChangeTaskEditRequestConsumed, refreshToken=0 }:{ onFocus:(id:string)=>void; onOpenHealth:()=>void; healthTaskDraft?:HealthTaskDraft|null; onHealthTaskDraftConsumed?:()=>void; planChangeTaskEditRequest?:PlanChangeTaskEditRequest|null; onPlanChangeTaskEditRequestConsumed?:()=>void; refreshToken?:number }) {
  ...
  const [tasksExpanded,setTasksExpanded]=useState(false); const [inboxExpanded,setInboxExpanded]=useState(false);
  ...
  useEffect(()=>{ const exact=tasks.filter(t=>t.scheduleKind==="exact"&&t.startAt&&minuteOf(t.startAt)>=TIMELINE_START_MINUTE&&minuteOf(t.startAt)<=TIMELINE_END_MINUTE).sort((a,b)=>minuteOf(a.startAt)-minuteOf(b.startAt)); const target=date===today()?Math.max(TIMELINE_START_MINUTE,Math.min(TIMELINE_END_MINUTE,minuteOf(new Date().toISOString()))):exact.length?minuteOf(exact[0]!.startAt):TIMELINE_START_MINUTE; requestAnimationFrame(()=>scrollRef.current?.scrollTo({top:Math.max(0,timelineTop(target)-160)})); },[date,tasks.length]);
  ...
  const exact=useMemo(()=>tasks.filter(t=>t.scheduleKind==="exact"&&t.startAt&&t.endAt&&t.lifecycleStatus!=="cancelled"&&minuteOf(t.startAt)>=TIMELINE_START_MINUTE&&minuteOf(t.endAt)<=TIMELINE_END_MINUTE),[tasks]);
  ...
  const displayedTasks=tasksExpanded?sortedTasks:sortedTasks.slice(0,3);
  ...
  function closeTaskActions(){setActionTaskId(null);setActionSurface(null);}
  function toggleTaskActions(taskId:string,surface:ActionSurface){const same=actionTaskId===taskId&&actionSurface===surface;setActionTaskId(same?null:taskId);setActionSurface(same?null:surface);}
  function openOutcome(task:Task){closeTaskActions();setOutcomeDraft({task,outcome:"complete",progress:"100",satisfaction:"satisfied",note:""});}
  ...
  function actionMenu(task:Task,surface:ActionSurface){if(actionTaskId!==task.id||actionSurface!==surface||task.lifecycleStatus==="active")return null;return <div className="task-action-menu" onPointerDown={e=>e.stopPropagation()}>{task.lifecycleStatus==="open"&&<button onClick={()=>{closeTaskActions();editTask(task);}}><Pencil/>编辑任务</button>}{task.lifecycleStatus==="open"&&<button onClick={()=>{closeTaskActions();onFocus(task.id);}}><Play/>开始专注</button>}{(task.lifecycleStatus==="open"||task.lifecycleStatus==="awaiting_outcome")&&<button onClick={()=>openOutcome(task)}><CheckCircle2/>记录结果</button>}<button onClick={()=>void openOutcomeHistory(task)}><History/>结果历史</button>{task.lifecycleStatus==="open"&&<button onClick={()=>void runTaskAction(task,"cancel")}><XCircle/>取消任务</button>}{(task.lifecycleStatus==="closed"||task.lifecycleStatus==="cancelled")&&<button onClick={()=>void runTaskAction(task,"reopen")}><RotateCcw/>重新打开</button>}<button className="danger" onClick={()=>void runTaskAction(task,"delete")}><Trash2/>移入回收站</button></div>;}
  ...
  function gridMinute(e:React.PointerEvent){const rect=e.currentTarget.getBoundingClientRect();return snapTimelineMinute(TIMELINE_START_MINUTE+(e.clientY-rect.top)/HOUR_PX*60);}
  ...
  return <section className="today-workspace">
    <header className="timeline-header"><div><p className="eyebrow">TODAY / 时间坐标</p><h1>把今天放回时间里。</h1><small className="timeline-rule">{date===today()?`可排时段 07:00–23:00；未来安排从 ${hhmm(earliestAllowedMinute(date))} 开始，过去时段可补录。`:"可排时段 07:00–23:00"}</small></div><div className="timeline-date"><input aria-label="时间轴日期" type="date" value={date} onChange={e=>setDate(e.target.value)}/><button className="primary-button" onClick={()=>{setError(null);setEditing(null);setSource(null);setForm(emptyForm(date));}}><CalendarPlus/>完整添加</button></div></header>
    ...
    <aside className="capture-panel"><section className="quick-capture"><div className="section-heading compact"><div><p className="section-kicker">快速记录</p><h2>先放进来</h2></div><Lightbulb/></div><div className="entry-switch">{(["task","idea","question"] as const).map(k=><button aria-pressed={quickKind===k} onClick={()=>setQuickKind(k)} key={k}>{k==="task"?"任务":k==="idea"?"想法":"问题"}</button>)}</div><form onSubmit={quickSave}><textarea aria-label="快速记录内容" value={quick} onChange={e=>setQuick(e.target.value)} rows={3} maxLength={200}/><button className="primary-button full-width" disabled={busy||!quick.trim()}>{busy?<LoaderCircle className="spin"/>:<Check/>}保存</button></form></section>
      <section className="task-dock"><header><div><ListTodo/><span><strong>任务列表</strong><small>{sortedTasks.length} 项，按时间排列</small></span></div><button className="trash-icon-button" type="button" onClick={()=>setTrashOpen(true)} aria-label={`打开回收站，${trash.length} 项`}><Trash2/><span>{trash.length}</span></button></header><div className="task-summary-list">{displayedTasks.length===0?<small>今天还没有任务。</small>:displayedTasks.map(t=><article key={t.id}><button type="button" onClick={()=>t.lifecycleStatus==="open"?editTask(t):toggleTaskActions(t.id,"list")}><span>{t.scheduleKind==="exact"&&t.startAt&&t.endAt?`${hhmm(minuteOf(t.startAt))}–${hhmm(minuteOf(t.endAt))}`:t.scheduleKind==="daypart"?({morning:"上午",afternoon:"下午",evening:"晚上"}[t.daypart!]):"未排期"}</span><strong>{t.title}</strong>{t.scheduleKind==="exact"&&t.startAt&&t.endAt&&(minuteOf(t.startAt)<TIMELINE_START_MINUTE||minuteOf(t.endAt)>TIMELINE_END_MINUTE)&&<small>历史任务：不在 07:00–23:00 可排时段</small>}</button>{t.lifecycleStatus!=="active"&&<button className="list-actions-toggle" onClick={()=>toggleTaskActions(t.id,"list")} aria-label={`打开 ${t.title} 的任务操作`}><MoreHorizontal/></button>}{actionMenu(t,"list")}</article>)}</div>{sortedTasks.length>3&&<button className="task-dock-toggle" type="button" onClick={()=>setTasksExpanded(value=>!value)}>{tasksExpanded?"收起其他任务":`查看其他 ${sortedTasks.length-3} 项`}<ChevronDown className={tasksExpanded?"expanded":""}/></button>}{tasksExpanded&&cancelled.length>0&&<div className="cancelled-summary"><p className="section-kicker">已取消</p>{cancelled.map(t=><article key={t.id}><strong>{t.title}</strong><button onClick={()=>void runTaskAction(t,"reopen")} aria-label={`重新打开 ${t.title}`}><RotateCcw/></button><button onClick={()=>void runTaskAction(t,"delete")} aria-label={`将 ${t.title} 移入回收站`}><Trash2/></button></article>)}</div>}</section>
    ...
    {trashOpen&&<div className="task-dialog-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setTrashOpen(false);}}><section className="trash-dialog" role="dialog" aria-modal="true" aria-labelledby="trash-dialog-title"><header><div><p className="section-kicker">回收站</p><h2 id="trash-dialog-title">查找并恢复任务</h2><small>删除的任务保留在数据库中，恢复时会重新检查版本与冲突。</small></div><button type="button" aria-label="关闭回收站" onClick={()=>setTrashOpen(false)}><X/></button></header><label className="trash-search"><Search/><input autoFocus value={trashQuery} onChange={e=>setTrashQuery(e.target.value)} placeholder="搜索任务标题或备注" aria-label="搜索回收站任务"/></label><div className="trash-results">{filteredTrash.length===0?<p>{trashQuery.trim()?"没有匹配的任务。":"回收站是空的。"}</p>:filteredTrash.map(task=><article key={task.id}><div><span>{task.scheduleKind==="exact"&&task.startAt?`${localDateOf(task.startAt)} ${hhmm(minuteOf(task.startAt))}`:task.localDate??"未排期"}</span><strong>{task.title}</strong>{task.notes&&<small>{task.notes}</small>}</div><button type="button" disabled={busy} onClick={()=>void runTaskAction(task,"restore")}><RotateCcw/>恢复</button></article>)}</div></section></div>}
