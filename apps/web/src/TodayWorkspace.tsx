...
export function TodayWorkspace({ onFocus, onOpenHealth, healthTaskDraft, onHealthTaskDraftConsumed, planChangeTaskEditRequest, onPlanChangeTaskEditRequestConsumed, refreshToken=0 }:{ onFocus:(id:string)=>void; onOpenHealth:()=>void; healthTaskDraft?:HealthTaskDraft|null; onHealthTaskDraftConsumed?:()=>void; planChangeTaskEditRequest?:PlanChangeTaskEditRequest|null; onPlanChangeTaskEditRequestConsumed?:()=>void; refreshToken?:number }) {
  ...
  async function submitOutcome(){if(!outcomeDraft)return;const progress=Number(outcomeDraft.progress);if(!validFocusEvaluation(outcomeDraft.outcome,outcomeDraft.progress)){setError("请填写与结果一致的客观进度。");return;}setBusy(true);setError(null);const completedTaskId=outcomeDraft.outcome==="complete"?outcomeDraft.task.id:null;try{
    if(outcomeDraft.mode==="edit"&&outcomeDraft.expectedOutcomeId){
      await json(`/api/v1/tasks/${outcomeDraft.task.id}/outcomes/correct`,`POST`,{expectedVersion:outcomeDraft.task.version,expectedOutcomeId:outcomeDraft.expectedOutcomeId,outcome:outcomeDraft.outcome,progressPercent:progress,source:"app",satisfaction:outcomeDraft.satisfaction,note:outcomeDraft.note.trim()||null});
  ...
  function actionMenu(task:Task,surface:ActionSurface){if(actionTaskId!==task.id||actionSurface!==surface||task.lifecycleStatus==="active"||!actionMenuPosition||typeof document==="undefined")return null;const factual=task.recordKind==="backfill";const canStartFocus=!factual&&task.lifecycleStatus==="open"&&task.scheduleKind==="exact"&&Boolean(task.startAt&&task.endAt);const todayEvaluation=date===today()&&(task.lifecycleStatus==="awaiting_outcome"||task.lifecycleStatus==="closed"||factual&&task.lifecycleStatus==="open");return createPortal(<div className="task-action-menu" style={{top:actionMenuPosition.top,left:actionMenuPosition.left}} onPointerDown={e=>e.stopPropagation()}>{!factual&&task.lifecycleStatus==="open"&&<button onClick={()=>{closeTaskActions();editTask(task);}}><Pencil/>编辑任务</button>}{canStartFocus&&<button onClick={()=>{closeTaskActions();onFocus(task.id);}}><Play/>开始专注</button>}{todayEvaluation&&<button onClick={()=>void openOutcome(task)}><CheckCircle2/>{task.lifecycleStatus==="closed"?"重新打开评价":"记录结果"}</button>}<button onClick={()=>void openOutcomeHistory(task)}><History/>结果历史</button>{!factual&&task.lifecycleStatus==="open"&&<button onClick={()=>void runTaskAction(task,"cancel")}><XCircle/>取消任务</button>}{!factual&&task.lifecycleStatus==="cancelled"&&<button onClick={()=>void runTaskAction(task,"reopen")}><RotateCcw/>重新启用任务</button>}<button className="danger" onClick={()=>void runTaskAction(task,"delete")}><Trash2/>移入回收站</button></div>,document.body);}
