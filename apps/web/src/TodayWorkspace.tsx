...
type FormState = { entryMode:"schedule"|"backfill"; origin:"manual"|"health"; title:string; scheduleKind:ScheduleKind; localDate:string; daypart:Daypart; start:string; end:string; notes:string };
...
const emptyForm = (date=today(), start?:string, end?:string, entryMode:FormState["entryMode"]="schedule"):FormState => {
  ...
  return { entryMode, origin:"manual", title:"", scheduleKind:"exact", localDate:date, daypart:"morning", start:start ?? hhmm(startMinute), end:end ?? hhmm(Math.min(LAST_TIMELINE_MINUTE,startMinute+TIMELINE_STEP_MINUTES)), notes:"" };
};
...
export function TodayWorkspace({ onFocus, onOpenHealth, healthTaskDraft, onHealthTaskDraftConsumed, refreshToken=0 }:{ onFocus:(id:string)=>void; onOpenHealth:()=>void; healthTaskDraft?:HealthTaskDraft|null; onHealthTaskDraftConsumed?:()=>void; refreshToken?:number }) {
  ...
  const load=useCallback(async()=>{ const requestId=++loadRequestRef.current;setError(null);try{const [taskData,inboxData,healthData]=await Promise.all([json<{tasks:Task[];blockingConflicts:Pair[];historicalOverlaps:Pair[]}>(`/api/v1/tasks?date=${date}`),json<InboxEntry[]>("/api/v1/inbox-entries"),json<{reference:HealthDaySummary|null}>(`/api/v1/health/days/${date}`).catch(()=>({reference:null}))]);if(requestId!==loadRequestRef.current)return;setTasks(taskData.tasks);setPairs(taskData.blockingConflicts);setHistory(taskData.historicalOverlaps);setInbox(inboxData);setHealthSummary(healthData.reference);}catch(error){if(requestId!==loadRequestRef.current)return;throw error;}},[date]);
  ...
  useEffect(()=>{if(!healthTaskDraft)return;setDate(healthTaskDraft.localDate);setEditing(null);setSource(null);setForm({entryMode:"schedule",origin:"health",title:healthTaskDraft.title,scheduleKind:"exact",localDate:healthTaskDraft.localDate,daypart:"morning",start:"",end:"",notes:healthTaskDraft.notes});onHealthTaskDraftConsumed?.();},[healthTaskDraft?.requestId]);
  ...
  const rangePreview=range?normalizedTimelineRange(range.start,range.current):null;
  ...
  function editTask(task:Task){setEditing(task);setSource(null);setForm({entryMode:"schedule",origin:"manual",title:task.title,scheduleKind:task.scheduleKind,localDate:task.localDate??date,daypart:task.daypart??"morning",start:hhmm(minuteOf(task.startAt)),end:hhmm(minuteOf(task.endAt)),notes:task.notes??""});}
  ...
  function actionMenu(task:Task){if(actionTaskId!==task.id||task.lifecycleStatus==="active")return null;return <div className="task-action-menu" onPointerDown={e=>e.stopPropagation()}>{task.lifecycleStatus==="open"&&<button onClick={()=>{setActionTaskId(null);onFocus(task.id);}}><Play/>开始专注</button>}{(task.lifecycleStatus==="open"||task.lifecycleStatus==="awaiting_outcome")&&<button onClick={()=>openOutcome(task)}><CheckCircle2/>记录结果</button>}<button onClick={()=>void openOutcomeHistory(task)}><History/>结果历史</button>{task.lifecycleStatus==="open"&&<button onClick={()=>void runTaskAction(task,"cancel")}><XCircle/>取消任务</button>}{task.lifecycleStatus==="open"&&<button className="danger" onClick={()=>void runTaskAction(task,"delete")}><Trash2/>删除任务</button>}{(task.lifecycleStatus==="closed"||task.lifecycleStatus==="cancelled")&&<button onClick={()=>void runTaskAction(task,"reopen")}><RotateCcw/>重新打开</button>}</div>;}
