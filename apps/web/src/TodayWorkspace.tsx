...
export function TodayWorkspace({ onFocus, onOpenHealth, healthTaskDraft, onHealthTaskDraftConsumed, planChangeTaskEditRequest, onPlanChangeTaskEditRequestConsumed, refreshToken=0 }:{ onFocus:(id:string)=>void; onOpenHealth:()=>void; healthTaskDraft?:HealthTaskDraft|null; onHealthTaskDraftConsumed?:()=>void; planChangeTaskEditRequest?:PlanChangeTaskEditRequest|null; onPlanChangeTaskEditRequestConsumed?:()=>void; refreshToken?:number }) {
  ...
  useEffect(()=>{ setFrontPeriod(defaultTimelinePeriod()); },[date]);
  ...
  const displayedScheduledTasks=tasksExpanded?scheduledTasks:scheduledTasks.slice(0,3);
  const displayedUnscheduledTasks=tasksExpanded?unscheduledTasks:unscheduledTasks.slice(0,3);
  const hiddenTaskCount=(scheduledTasks.length-displayedScheduledTasks.length)+(unscheduledTasks.length-displayedUnscheduledTasks.length);
  ...
  async function saveForm(e:React.FormEvent){
    ...
    try{
      ...
      if(form.entryMode==="backfill"&&created)setOutcomeDraft({task:created,outcome:"complete",progress:"100",satisfaction:"satisfied",note:""});
  ...
  async function submitOutcome(e:React.FormEvent){e.preventDefault();if(!outcomeDraft)return;const progress=Number(outcomeDraft.progress);if(!Number.isInteger(progress)||(outcomeDraft.outcome==="complete"&&progress!==100)||(outcomeDraft.outcome==="not_completed"&&progress!==0)||(outcomeDraft.outcome==="partial"&&(progress<1||progress>99))){setError("请填写与结果一致的客观进度。");return;}setBusy(true);setError(null);const completedTaskId=outcomeDraft.outcome==="complete"?outcomeDraft.task.id:null;try{await json(`/api/v1/tasks/${outcomeDraft.task.id}/outcomes`,`POST`,{expectedVersion:outcomeDraft.task.version,outcome:outcomeDraft.outcome,progressPercent:progress,source:"app",satisfaction:outcomeDraft.satisfaction,note:outcomeDraft.note.trim()||null});setOutcomeDraft(null);await load();if(completedTaskId){setCompletedTaskSealId(completedTaskId);feedbackTimersRef.current.push(window.setTimeout(()=>setCompletedTaskSealId(null),900));}}catch{setError("任务结果没有保存，请刷新后重试。");}finally{setBusy(false);}}
  ...
  function actionMenu(task:Task,surface:ActionSurface){if(actionTaskId!==task.id||actionSurface!==surface||task.lifecycleStatus==="active")return null;const factual=task.recordKind==="backfill";const canStartFocus=!factual&&task.lifecycleStatus==="open"&&task.scheduleKind==="exact"&&Boolean(task.startAt&&task.endAt);return <div className="task-action-menu" onPointerDown={e=>e.stopPropagation()}>{!factual&&task.lifecycleStatus==="open"&&<button onClick={()=>{closeTaskActions();editTask(task);}}><Pencil/>编辑任务</button>}{canStartFocus&&<button onClick={()=>{closeTaskActions();onFocus(task.id);}}><Play/>开始专注</button>}{(task.lifecycleStatus==="open"||task.lifecycleStatus==="awaiting_outcome")&&<button onClick={()=>openOutcome(task)}><CheckCircle2/>记录结果</button>}<button onClick={()=>void openOutcomeHistory(task)}><History/>结果历史</button>{!factual&&task.lifecycleStatus==="open"&&<button onClick={()=>void runTaskAction(task,"cancel")}><XCircle/>取消任务</button>}{!factual&&(task.lifecycleStatus==="closed"||task.lifecycleStatus==="cancelled")&&<button onClick={()=>void runTaskAction(task,"reopen")}><RotateCcw/>重新打开</button>}<button className="danger" onClick={()=>void runTaskAction(task,"delete")}><Trash2/>移入回收站</button></div>;}
  ...
  function openTaskFromList(task:Task){if(suppressTaskClickRef.current===task.id){suppressTaskClickRef.current=null;return;}if(task.recordKind==="formal"&&task.lifecycleStatus==="open")editTask(task);else toggleTaskActions(task.id,"list");}
  ...
  function renderTaskListRow(t:Task){
    ...
    return <article className={`${t.recordKind==="backfill"?"fact-record-row":""} ${draggable?"unscheduled-draggable":""} ${placementPreview?.taskId===t.id?"placement-source":""} ${completedTaskSealId===t.id?"completion-seal-landing":""}`} key={t.id}>
      ...
      {t.lifecycleStatus!=="active"&&<button className="list-actions-toggle" onClick={()=>toggleTaskActions(t.id,"list")} aria-label={`打开 ${t.title} 的${t.recordKind==="backfill"?"事实记录":"任务"}操作`}><MoreHorizontal/></button>}
      {actionMenu(t,"list")}
    </article>;
  }
  ...
  return <section className="today-workspace">
    ...
    <aside className="capture-panel"><section className="quick-capture"><div className="section-heading compact"><div><p className="section-kicker">快速记录</p><h2>先放进来</h2></div>{quickKind==="task"?<button className="quick-full-add" type="button" onClick={()=>{setError(null);setEditing(null);setSource(null);setForm(emptyForm(date));}}><CalendarPlus/>完整添加</button>:<Lightbulb className="quick-lightbulb" aria-hidden="true"/>}</div><div className="entry-switch">{(["task","idea","question"] as const).map(k=><button aria-pressed={quickKind===k} onClick={()=>setQuickKind(k)} key={k}>{k==="task"?"任务":k==="idea"?"想法":"问题"}</button>)}</div><form onSubmit={quickSave}><textarea aria-label="快速记录内容" placeholder={quickKind==="task"?"写下一件准备完成的事……":quickKind==="idea"?"先记下灵光，不必现在整理……":"把仍待解决的问题放在这里……"} value={quick} onChange={e=>setQuick(e.target.value)} rows={3} maxLength={200}/><button className="primary-button full-width" disabled={busy||!quick.trim()}>{busy?<LoaderCircle className="spin"/>:<Check/>}保存</button></form></section>
      <section className="task-dock"><header><div><ListTodo/><span><strong>任务列表</strong><small>已排期 {scheduledTasks.length} · 未排期 {unscheduledTasks.length}</small></span></div><button className="trash-icon-button" type="button" onClick={()=>setTrashOpen(true)} aria-label={`打开回收站，${trash.length} 项`}><Trash2/><span>{trash.length}</span></button></header><div className="task-summary-list">
        <section className="task-summary-group scheduled" aria-labelledby="scheduled-task-heading"><div className="task-summary-group-heading"><strong id="scheduled-task-heading">已排期任务</strong><span>{scheduledTasks.length} 项</span></div>{displayedScheduledTasks.length===0?<small className="task-summary-empty">今天还没有已排期任务。</small>:displayedScheduledTasks.map(renderTaskListRow)}</section>
        <section className="task-summary-group unscheduled" aria-labelledby="unscheduled-task-heading"><div className="task-summary-group-heading"><span><strong id="unscheduled-task-heading">未排期任务</strong><small>可拖入时间轴</small></span><span>{unscheduledTasks.length} 项</span></div>{displayedUnscheduledTasks.length===0?<small className="task-summary-empty">今天还没有未排期任务。</small>:displayedUnscheduledTasks.map(renderTaskListRow)}</section>
      </div>{hiddenTaskCount>0||tasksExpanded?<button className="task-dock-toggle" type="button" onClick={()=>setTasksExpanded(value=>!value)}>{tasksExpanded?"收起其他任务":`查看其他 ${hiddenTaskCount} 项`}<ChevronDown className={tasksExpanded?"expanded":""}/></button>:null}{tasksExpanded&&cancelled.length>0&&<div className="cancelled-summary"><p className="section-kicker">已取消</p>{cancelled.map(t=><article key={t.id}><strong>{t.title}</strong><button onClick={()=>void runTaskAction(t,"reopen")} aria-label={`重新打开 ${t.title}`}><RotateCcw/></button><button onClick={()=>void runTaskAction(t,"delete")} aria-label={`将 ${t.title} 移入回收站`}><Trash2/></button></article>)}</div>}</section>
