...
import { AlertTriangle, CalendarPlus, Check, CheckCircle2, CircleDashed, GripHorizontal, Lightbulb, LoaderCircle, MoreHorizontal, Pencil, Play, RotateCcw, Sparkles, Trash2, X, XCircle } from "lucide-react";
...
type FormState = { title:string; scheduleKind:ScheduleKind; localDate:string; daypart:Daypart; start:string; end:string; notes:string };
...
type OutcomeDraft = { task:Task; outcome:"not_completed"|"partial"|"complete"; progress:string; note:string };
...
const emptyForm = (date=today(), start?:string, end?:string):FormState => {
  ...
  return { title:"", scheduleKind:"exact", localDate:date, daypart:"morning", start:start ?? hhmm(startMinute), end:end ?? hhmm(Math.min(LAST_TIMELINE_MINUTE,startMinute+TIMELINE_STEP_MINUTES)), notes:"" };
};
...
export function TodayWorkspace({ onFocus, refreshToken=0 }:{ onFocus:(id:string)=>void; refreshToken?:number }) {
  ...
  async function withConflict(path:string,method:string,payload:Record<string,unknown>) { let currentPayload=payload;for(let attempt=0;attempt<3;attempt+=1){try{return await json<any>(path,method,currentPayload);}catch(e:any){if(!["task_time_conflict","conflict_set_changed"].includes(e.body?.error)||!e.body.conflictSetFingerprint)throw e;if(!window.confirm("该时段与现有任务重叠。是否明确保留当前全部冲突？"))throw e;const decision={conflictDecision:"keep",expectedConflictFingerprint:e.body.conflictSetFingerprint};currentPayload=path.includes("/inbox-entries/")?{...payload,task:{...(payload.task as Record<string,unknown>),...decision}}:{...payload,...decision};}}throw new Error("conflict_set_kept_changing"); }
  ...
  async function saveForm(e:React.FormEvent){e.preventDefault();if(!form?.title.trim())return;if(form.scheduleKind==="exact"){const start=Number(form.start.slice(0,2))*60+Number(form.start.slice(3));const end=Number(form.end.slice(0,2))*60+Number(form.end.slice(3));if(!isTimelineMinute(start)||!isTimelineMinute(end)){setError("精确任务的开始和结束时间必须使用 30 分钟间隔。");return;}if(end-start<TIMELINE_STEP_MINUTES){setError("精确任务的结束时间必须至少晚于开始时间 30 分钟。");return;}if(end>LAST_TIMELINE_MINUTE){setError("第一版不支持跨越午夜，最晚结束时间为 23:30。");return;}const earliest=earliestAllowedMinute(form.localDate);if(start<earliest){setError(`今天当前时间段已不可用，请从 ${hhmm(earliest)} 开始安排。`);return;}}setBusy(true);setError(null);try{const taskPayload=payload(form); if(source){await withConflict(`/api/v1/inbox-entries/${source.id}/convert-to-task`,`POST`,{confirmed:true,expectedVersion:source.version,task:taskPayload});}else if(editing){await withConflict(`/api/v1/tasks/${editing.id}`,`PATCH`,{expectedVersion:editing.version,expectedScheduleRevision:editing.scheduleRevision,...taskPayload});}else{await withConflict("/api/v1/tasks","POST",taskPayload);}setForm(null);setEditing(null);setSource(null);await load();}catch(err:any){setError(err.body?.error==="inbox_entry_conflict"?"这条想法已被其他位置更新或转换，请刷新后重新整理。":err.body?.error==="task_schedule_revision_conflict"?"排期已在其他位置更新，请重新打开。":"保存失败，原内容仍在表单中。");}finally{setBusy(false);}}
  ...
  function editTask(task:Task){setEditing(task);setSource(null);setForm({title:task.title,scheduleKind:task.scheduleKind,localDate:task.localDate??date,daypart:task.daypart??"morning",start:hhmm(minuteOf(task.startAt)),end:hhmm(minuteOf(task.endAt)),notes:task.notes??""});}
  function openOutcome(task:Task){setActionTaskId(null);setOutcomeDraft({task,outcome:"complete",progress:"100",note:""});}
  ...
  async function submitOutcome(e:React.FormEvent){e.preventDefault();if(!outcomeDraft)return;const progress=Number(outcomeDraft.progress);if(!Number.isInteger(progress)||(outcomeDraft.outcome==="complete"&&progress!==100)||(outcomeDraft.outcome==="not_completed"&&progress!==0)||(outcomeDraft.outcome==="partial"&&(progress<1||progress>99))){setError("请填写与结果一致的客观进度。");return;}setBusy(true);setError(null);try{await json(`/api/v1/tasks/${outcomeDraft.task.id}/outcomes`,`POST`,{expectedVersion:outcomeDraft.task.version,outcome:outcomeDraft.outcome,progressPercent:progress,source:"app",note:outcomeDraft.note.trim()||null});setOutcomeDraft(null);await load();}catch{setError("任务结果没有保存，请刷新后重试。");}finally{setBusy(false);}}
  async function runTaskAction(task:Task,action:"cancel"|"delete"|"reopen"){if(action==="delete"&&!window.confirm("删除后任务将从正常时间轴隐藏。是否继续？"))return;setBusy(true);setError(null);try{if(action==="cancel")await json(`/api/v1/tasks/${task.id}/cancel`,`POST`,{expectedVersion:task.version});else if(action==="delete")await json(`/api/v1/tasks/${task.id}`,`DELETE`,{expectedVersion:task.version});else await withConflict(`/api/v1/tasks/${task.id}/reopen`,`POST`,{expectedVersion:task.version});setActionTaskId(null);await load();}catch(err:any){setError(err.body?.error==="task_version_conflict"?"任务已在其他位置更新，请刷新后重试。":"操作没有保存，任务状态保持不变。");}finally{setBusy(false);}}
  function outcomeLabel(task:Task){return task.currentOutcome==="complete"?"已完成":task.currentOutcome==="partial"?"部分完成":task.currentOutcome==="not_completed"?"未完成":"已结束";}
  function actionMenu(task:Task){if(actionTaskId!==task.id||task.lifecycleStatus==="active")return null;return <div className="task-action-menu" onPointerDown={e=>e.stopPropagation()}>{task.lifecycleStatus==="open"&&<button onClick={()=>{setActionTaskId(null);onFocus(task.id);}}><Play/>开始专注</button>}{(task.lifecycleStatus==="open"||task.lifecycleStatus==="awaiting_outcome")&&<button onClick={()=>openOutcome(task)}><CheckCircle2/>记录结果</button>}{task.lifecycleStatus==="open"&&<button onClick={()=>void runTaskAction(task,"cancel")}><XCircle/>取消任务</button>}{task.lifecycleStatus==="open"&&<button className="danger" onClick={()=>void runTaskAction(task,"delete")}><Trash2/>删除任务</button>}{(task.lifecycleStatus==="closed"||task.lifecycleStatus==="cancelled")&&<button onClick={()=>void runTaskAction(task,"reopen")}><RotateCcw/>重新打开</button>}</div>;}
  ...
  function endRange(e:React.PointerEvent<HTMLDivElement>){const current=rangeRef.current;if(!current||current.pointerId!==e.pointerId)return;rangeRef.current=null;setRange(null);const low=Math.min(current.start,current.current);const high=Math.max(current.start,current.current);const earliest=earliestAllowedMinute(date);if(low<earliest){setError(`今天当前时间段已不可用，请从 ${hhmm(earliest)} 开始安排。`);return;}const hasRange=high-low>=TIMELINE_STEP_MINUTES;const start=hasRange?Math.min(low,LAST_TIMELINE_MINUTE-TIMELINE_STEP_MINUTES):Math.min(low,LAST_TIMELINE_MINUTE-TIMELINE_STEP_MINUTES);const end=hasRange?Math.min(LAST_TIMELINE_MINUTE,Math.max(start+TIMELINE_STEP_MINUTES,high)):start+TIMELINE_STEP_MINUTES;setEditing(null);setSource(null);setForm(emptyForm(date,hhmm(start),hhmm(end)));}

  return <section className="today-workspace">
    <header className="timeline-header"><div><p className="eyebrow">TODAY / 时间坐标</p><h1>把今天放回时间里。</h1><small className="timeline-rule">今天只能从当前半小时之后开始安排：{date===today()?hhmm(earliestAllowedMinute(date)):"全天可选"}</small></div><div className="timeline-date"><input aria-label="时间轴日期" type="date" value={date} onChange={e=>setDate(e.target.value)}/><button className="primary-button" onClick={()=>{setEditing(null);setSource(null);setForm(emptyForm(date));}}><CalendarPlus/>完整添加</button></div></header>
    ...
    <aside className="capture-panel"><section className="quick-capture"><div className="section-heading compact"><div><p className="section-kicker">快速记录</p><h2>先放进来</h2></div><Lightbulb/></div><div className="entry-switch">{(["task","idea","question"] as const).map(k=><button aria-pressed={quickKind===k} onClick={()=>setQuickKind(k)} key={k}>{k==="task"?"任务":k==="idea"?"想法":"问题"}</button>)}</div><form onSubmit={quickSave}><textarea aria-label="快速记录内容" value={quick} onChange={e=>setQuick(e.target.value)} rows={3} maxLength={200}/><button className="primary-button full-width" disabled={busy||!quick.trim()}>{busy?<LoaderCircle className="spin"/>:<Check/>}保存</button></form></section>
      ...
      {cancelled.length>0&&<section className="cancelled-tasks"><p className="section-kicker">已取消</p>{cancelled.map(t=><article key={t.id}><span>{t.scheduleKind==="exact"&&t.startAt?hhmm(minuteOf(t.startAt)):"待定"}</span><strong>{t.title}</strong><button onClick={()=>void runTaskAction(t,"reopen")} aria-label={`重新打开 ${t.title}`}><RotateCcw/></button></article>)}</section>}
