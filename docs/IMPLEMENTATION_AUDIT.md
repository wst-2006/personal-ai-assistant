...
| Daily brief | Product Spec, Roadmap | Partially implemented | Medium for review-derived draft, dynamic location and provider boundaries; medium for local export/edit acceptance; low for live external acceptance | Review generates, edits, regenerates, confirms, exports and reloads a persisted brief with finance, AI, technology, task-expansion, humanities, weather and location sections. Confirmed content is read-only until the user explicitly enters edit mode; edits persist through the brief API. The user enters that day's location in the review page; geocoded location and weather snapshots persist in brief JSON. | `brief-providers.ts`, brief routes, `ReviewWorkspace.tsx`; `daily_briefs.content`, `daily_briefs.sources` | domain/provider tests, 390px browser E2E and build checks | Add opted-in device location, live-provider acceptance and normal-chat path. |
| Cyber diary | Product Spec, State Machines | Partially implemented | High for the local persistence and derived-data browser path; medium for the complete diary experience | API-backed draft, save, reload, edit and text export. Saving requires a same-day review session with at least one message and a confirmed brief linked to that review. The page now derives task/focus cards, raw and effective focus, location/weather, six daily metrics, state color and the day's tree from PostgreSQL records and the linked confirmed brief. | `diary-routes.ts`, `diary-service.ts`, `DiaryWorkspace.tsx`; `cyber_diaries`, `review_sessions`, `review_messages`, `daily_briefs`, task/focus/outcome/feedback tables | diary domain and day-data calculation tests; guarded browser E2E for save/reload/export/390px with UUID-only cleanup | Add month navigation and decide whether final diary exports need an immutable derived-data snapshot rather than authoritative live derivation. |
...
| Search, weather, location, export | Product Spec, Roadmap | Partially implemented | Medium for adapter boundaries and user-entered location; low for live external acceptance | Free GDELT news search is the default; Brave is optional; Open-Meteo geocoding/weather is server-only, location is entered per day, and source/snapshot metadata is retained. | `brief-providers.ts`, `ReviewWorkspace.tsx`; `daily_briefs.content`, `daily_briefs.sources` | provider tests, browser input/mobile E2E and type checks | Add opted-in device location, live-provider acceptance and brief export. |
...
| Review | Opens/restores a daily review session, saves messages, reads real context, generates/edits/confirms/regenerates/exports a brief, and keeps confirmed brief edits persisted | Software conversation context and full browser acceptance | Partially implemented |
| Diary | Reads its review/brief prerequisites, saves, restores, edits and exports a persisted diary with server-side link validation; diary creation remains blocked until a review message and confirmed brief exist. Real task/focus cards, weather/location, six metrics, daily state color and tree feedback are API-derived. | Month navigation and an immutable final export snapshot remain | Partially implemented |
...
6. **Growth feedback is intentionally basic.** It now derives from persisted
   ...
   implemented.
7. **Cyber diary history remains narrow.** Referential integrity, task/focus
   cards, weather/location, six metrics, state color and tree data are now
   verified against the guarded database. Month navigation and a product
   decision on immutable final snapshots remain.
