...
| Daily brief | Product Spec, Roadmap | Partially implemented | High for local persistence, structured DeepSeek generation, source retention, edit/confirm/export and desktop/390px paths; medium for complete provider/location coverage | Review generation now sends bounded review/task/search material to a server-only DeepSeek structured writer. The validated result contains finance, AI, technology, task-related expansion, humanities/society and encouragement sections; weather/location remains a separate provider snapshot. Search URLs/provider metadata are retained and displayed as links. Provider or malformed-output failure returns `503 brief_generation_unavailable` without persisting an incomplete result. A normal conversation still uses a separate explicit AI-drawer action to create a confirmed standalone brief with `reviewSessionId = null`; it is listed, restored after refresh and exportable, and never qualifies for a cyber diary. Confirmed review content remains read-only until explicit edit mode; title, section titles/bodies and summaries persist through the brief API. | `brief-service.ts`, `brief-routes.ts`, `ai/brief-writer.ts`, `App.tsx`, `ReviewWorkspace.tsx`; `daily_briefs.content`, `daily_briefs.sources`, nullable `daily_briefs.review_session_id` | `ai/brief-writer.test.ts`, domain contract tests, API 503 route test, review and standalone PostgreSQL/browser E2E (including 390px), provider tests and build checks | Add opted-in device location and broader live provider/model acceptance; keep standalone briefs excluded from diary prerequisites. |
