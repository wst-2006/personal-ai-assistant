...
| Daily brief | Product Spec, Roadmap | Partially implemented | Medium for review-derived draft and explicit standalone persistence; medium for local export/edit/browser acceptance; low for live external-provider acceptance | Review generates, edits, regenerates, confirms, exports and reloads a persisted brief with finance, AI, technology, task-expansion, humanities, weather and location sections. A normal conversation can now use a separate explicit AI-drawer action to create a confirmed standalone brief with `reviewSessionId = null`; it is listed, restored after refresh and exportable, and never qualifies for a cyber diary. Confirmed review content remains read-only until the user explicitly enters edit mode; edits persist through the brief API. | `brief-service.ts`, `brief-routes.ts`, `App.tsx`, `ReviewWorkspace.tsx`; `daily_briefs.content`, `daily_briefs.sources`, nullable `daily_briefs.review_session_id` | domain contract tests, API route tests, standalone PostgreSQL/browser E2E (including 390px) and build checks | Add opted-in device location and live external-provider acceptance; keep standalone briefs excluded from diary prerequisites. |
...
| Search, weather, location, export | Product Spec, Roadmap | Partially implemented | Medium for adapter boundaries and user-entered location; low for live external acceptance | Tavily is the preferred optional LLM-oriented provider, Brave is the secondary optional provider, and free GDELT remains the no-key fallback. Open-Meteo geocoding/weather is server-only; location is entered per day, and source/snapshot metadata is retained. | `brief-providers.ts`, `ReviewWorkspace.tsx`; `daily_briefs.content`, `daily_briefs.sources` | provider tests, browser input/mobile E2E and type checks | Configure and acceptance-test a real Tavily key; add opted-in device location and export acceptance. |
