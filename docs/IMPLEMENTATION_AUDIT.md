...
| AI candidate confirmation | Product Spec, Design Review | Partially implemented | High for local parse/edit/confirm/persistence and framework task-tree confirmation; medium for live provider failures | The AI drawer parses a transient editable task, idea, or question candidate. The Planning workspace separately persists editable framework-level task-tree candidates. Neither path creates a task before the user explicitly confirms it. Task-tree confirmation creates only unscheduled open tasks, records their long-range-plan source, and does not alter the plan or timeline. | `POST /ai/tasks/parse`, `POST /tasks`, `POST /inbox-entries`, task-tree candidate routes; `tasks`, `inbox_entries`, `long_range_plan_task_tree_candidates` | parser/API/service tests and real PostgreSQL/browser E2E: no pre-confirmation write, candidate edit + refresh, explicit confirmation, source-linked task persistence, and 390px smoke | Add explicit browser coverage for provider-unavailable and user-declined conflict paths; verify a live DeepSeek structured response before increasing provider confidence. |
| Real Today timeline | Product Spec, Design Review | Partially implemented | High for core schedule editing and keyboard movement; medium for full interaction | 24-hour coordinate axis, current-time marker, daypart/unscheduled areas, creation, pointer drag/resize, keyboard move/resize and spatial overlaps | task APIs; `tasks`, conflict acceptances | desktop and 390px browser E2E, including persisted keyboard movement/resize | Add richer accessible conflict explanations. |
...
| Focus session workflow | Product Spec, State Machines, Roadmap | Partially implemented | High for core persistence and desktop/390px browser paths; medium for external interaction | API-backed preparation, reminder response, auto-start, fixed-end timing, persisted structure execution, end, objective outcome and subjective feedback; exact tasks transactionally schedule a 15-minute reminder. A five-minute unanswered reminder atomically stops its session, appends a system `not_completed` outcome, closes the task, and releases the next focus selection. Choosing “另有安排” now stops only the reminder and opens a contextual AI consultation; it cannot alter the task until the user returns to the timeline and explicitly saves a change. | focus-session routes, `POST /ai/plan-change-advisories`; `focus_sessions`, `task_feedback`, `task_outcomes`, `reminder_jobs`, `focus_structures` | domain validation, database reminder verification, Worker tests, API advisory tests, desktop/390px browser E2E including no-response persistence and no-write plan-change consultation | Complete local end-to-end delivery if enabled. Pause/resume and manual restart are intentionally excluded by the current product decision. Remote delivery is deferred. |
...
| Growth and statistics | Product Spec, Roadmap, Design Review | Partially implemented | High for the seven-day and thirty-day database-derived paths; low for long-term trends | Real seven-day and thirty-day focus trends, subjective feedback counts, daily state grids, six metrics, points and tree feedback | `growth-routes.ts`, `growth-service.ts`, `GrowthWorkspace.tsx`; task/focus/outcome/feedback/review tables | guarded API contract tests, real PostgreSQL-backed summary checks, desktop and 390px browser E2E | Add longer-range trend exploration and decide whether persistent derived snapshots are justified. |
...
| Local Worker and optional Feishu | Architecture, Roadmap | Partially implemented | High for guarded local queue contract and five-minute recovery; medium for adapter/webhook tests; low for live delivery | Exact-task writes transactionally create/update/cancel revision-bound reminder jobs. The local worker claims atomically, rejects stale tasks, retries failures, finalizes unanswered focus reminders with a system task outcome, obtains Feishu tenant tokens and sends interactive cards when configured. | `reminder_jobs`, `focus_timer_jobs`; `reminder-scheduler.ts`, `focus-no-response.ts`, `apps/worker`, `feishu-webhook.ts` | migration guard/schema and database persistence verification; Worker/provider/webhook tests, real PostgreSQL/browser no-response E2E, and builds | Complete local end-to-end delivery if enabled. Cloud deployment is out of scope. |
| Weekly health reference | Latest health-reference baseline | Partially implemented | High for the complete user-maintained profile form, manually editable/AI/template/sleep-revision candidates, versioned confirmation, PostgreSQL persistence and desktop/390px browser flows; low for provider-dependent visual extraction | Separate Health page lets the user manually edit every health-profile field covered by the shared contract, and directly edit a complete seven-day food/movement candidate. Editing an existing template, AI, or sleep-based candidate atomically updates that candidate with optimistic versioning; it never replaces the active week until an explicit confirmation. User-selected PNG/JPEG/WebP sleep screenshots are validated, sent transiently through a replaceable vision adapter, and persisted only as a hash plus structured visible metrics. City is intentionally optional and never inferred. | `health-routes.ts`, `health-service.ts`, `health-planner.ts`, `sleep-image-analyzer.ts`, `HealthWorkspace.tsx`; `health_profiles`, `health_week_plans`, `health_daily_references`, `health_sleep_analyses` | domain/schema/guarded PostgreSQL tests, API route tests, browser E2E for complete profile field mapping, manual edit/save/refresh/confirm flow, candidate/confirmation/no-task writes, sleep revision confirmation, upload UX and 390px | Configure and verify a vision-capable provider model; extend only with health requirements that preserve the explicit save/candidate/confirmation boundary. |
...
| --- | --- | --- | --- |
| Today | Database-backed task/inbox capture, complete form, 30-minute time axis/interactions, blank-slot creation, pointer and keyboard drag/resize, conflict retention, lifecycle actions and direct task-to-focus handoff | Rich accessible conflict detail and cloud synchronization | Partially implemented |
...
| Health | A complete user-controlled profile form, active/candidate weekly references, structured sleep screenshot analyses, and explicit source-linked sleep revision candidates are API-backed and restored from PostgreSQL. The user can open the manual editor from the visible plan, change every weekly overview/day-range field, save a versioned manual candidate, refresh it, and explicitly confirm it. The default local candidate gives only ranges, plate guidance, seasonal vegetables, category/duration/intensity and safety prompts; it never creates health tasks or scores. DeepSeek refinement, vision extraction, and sleep revision remain separate from confirmed plan data until the user confirms a candidate. | A vision-capable provider model still needs real credential/model verification; no Today summary card by design in this slice | Partially implemented |
...
| AI drawer | Server-side model parsing returns a transient candidate with editable task or inbox fields; only explicit confirmation writes through the real API, and saved tasks persist through refresh. A separate Planning path persists framework task-tree candidates and, after a separate explicit confirmation, creates only unscheduled source-linked tasks. A standalone-brief action persists a confirmed brief and restores it on reopen/refresh without creating a review or diary. “另有安排” opens a task/day-context advisory that returns visible options only and never writes a schedule change. | no durable conversation context; provider-unavailable UI and a live DeepSeek task-tree response need dedicated acceptance | Partially implemented |
...
6. **Growth feedback is intentionally basic.** It now derives from persisted
   records for selectable seven-day and thirty-day windows, but year/long-term
   trends and a deeper statistical model are not yet implemented.
...
8. **Later dependencies remain absent.** Local Feishu controls, PWA, and
   cross-device verification still need explicit implementation phases; cloud
   runtime is deferred and the durable local Worker queue now exists.
...
11. **Live DeepSeek provider acceptance is still unresolved.** A real,
    no-write task-candidate request reached the configured provider but was
    rejected with HTTP 400 (the application surfaced 502). Code-level
    structured-output and no-write behavior remain tested; model/key/quota
    configuration must be corrected before claiming live provider confidence.
