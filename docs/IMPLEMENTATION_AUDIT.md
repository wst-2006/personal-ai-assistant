...
| Weekly health reference | Latest health-reference baseline | Partially implemented | High for the complete user-maintained profile form, plan versions, explicit confirmation, source-linked sleep revisions, PostgreSQL persistence and desktop/390px browser flows; low for provider-dependent visual extraction | Separate Health page lets the user manually edit every health-profile field covered by the shared contract, presents a seven-day read-only food/movement-range candidate, lets the user optionally request an explicit DeepSeek refinement, and requires confirmation before a candidate becomes the active week. User-selected PNG/JPEG/WebP sleep screenshots are validated, sent transiently through a replaceable vision adapter, and persisted only as a hash plus structured visible metrics. A user can explicitly request a sleep-based revision candidate; it records the prior plan version and source analysis, displays a reason and before/after differences, and cannot replace the active plan without confirmation. City is intentionally optional and never inferred. | `health-routes.ts`, `health-service.ts`, `health-planner.ts`, `sleep-image-analyzer.ts`, `HealthWorkspace.tsx`; `health_profiles`, `health_week_plans`, `health_daily_references`, `health_sleep_analyses` | domain/schema/guarded PostgreSQL tests, API route tests, browser E2E for complete profile field mapping, candidate/confirmation/refresh/no task writes, sleep revision confirmation, upload UX and 390px | Configure and verify a vision-capable provider model; add richer manual weekly editing only if it can preserve the same versioned candidate/confirmation boundary. |
...
| Health | A complete user-controlled profile form, active/candidate weekly references, structured sleep screenshot analyses, and explicit source-linked sleep revision candidates are API-backed and restored from PostgreSQL. The default local candidate gives only ranges, plate guidance, seasonal vegetables, category/duration/intensity and safety prompts; it never creates health tasks or scores. DeepSeek refinement, vision extraction, and sleep revision are explicit and remain separate from confirmed plan data until the user confirms a candidate. | A vision-capable provider model still needs real credential/model verification; richer manual weekly editing remains unimplemented; no Today summary card by design in this slice | Partially implemented |
