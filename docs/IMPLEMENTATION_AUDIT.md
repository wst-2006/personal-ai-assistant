...
| Health | User-controlled profile, active/candidate weekly references, structured sleep screenshot analyses, and explicit source-linked sleep revision candidates are API-backed and restored from PostgreSQL. The default local candidate gives only ranges, plate guidance, seasonal vegetables, category/duration/intensity and safety prompts; it never creates health tasks or scores. DeepSeek refinement, vision extraction, and sleep revision are explicit and remain separate from confirmed plan data until the user confirms a candidate. | A vision-capable provider model still needs real credential/model verification; richer manual weekly editing remains unimplemented; no Today summary card by design in this slice | Partially implemented |
