...
| AI candidate confirmation | Product Spec, Design Review | Partially implemented | High for local parse/edit/confirm/persistence path; medium for provider failure and task-tree work | AI drawer parses into a transient editable candidate; the user can correct kind, title, schedule/date/time or notes before an explicit confirmation. Formal tasks use the strict current task contract; ideas/questions show no task fields and write only through the independent inbox API. The parse endpoint has no persistence side effect. | `POST /ai/tasks/parse`, `POST /tasks`, `POST /inbox-entries`; `tasks`, `inbox_entries` | parser/API contract tests and `ai-task-candidate.spec.ts` real PostgreSQL/browser E2E: no pre-confirmation write, edited task persistence + refresh, independent idea persistence, and 390px smoke | Add explicit browser coverage for provider-unavailable and user-declined conflict paths; add durable conversation context and the separately approved AI task-tree candidate/confirmation flow. |
...
| Long-range planning and AI task trees | Roadmap, Product Spec | Partially implemented | High for manual long-range plans; high that AI tree writes remain absent | The new Planning workspace lets the user create, edit, archive and restore monthly mainlines, semester plans and annual directions with ordered self-owned milestones. It never automatically creates tasks, changes the daily timeline or invokes AI. | `long-range-plan-routes.ts`, `long-range-plan-service.ts`, `LongRangePlansWorkspace.tsx`; `long_range_plans`, `long_range_plan_milestones` | schema/guard and service tests; real PostgreSQL browser E2E for create, refresh, edit, stale-version error, archive/restore, UUID-only cleanup and 390px smoke | Implement a read-only AI framework-decomposition candidate next. Confirmation must be explicit and write each child task with normal version/conflict checks; no candidate may mutate a plan or task before confirmation. |
...
| Planning | Real monthly, semester and annual plans with ordered milestones load from PostgreSQL, retain user edits after refresh, use optimistic versions, and require explicit archive/restore actions. The page does not create a task or call AI. | AI framework-level task-tree candidate and explicit per-task confirmation are still absent. | Partially implemented |
| AI drawer | Server-side model parsing returns a transient candidate with editable task or inbox fields; only explicit confirmation writes through the real API, and saved tasks persist through refresh. A separate explicit standalone-brief action persists a confirmed brief and restores it on reopen/refresh without creating a review or diary. “另有安排” opens a task/day-context advisory that returns visible options only and never writes a schedule change. | no durable conversation context or long-range task-tree confirmation flow; provider-unavailable UI needs dedicated browser coverage | Partially implemented |
...
The plan API is `GET/POST /long-range-plans`,
`GET/PUT /long-range-plans/:id`, and
`POST /long-range-plans/:id/status`. No endpoint in this slice writes a daily
task, uses AI, or changes a schedule.
