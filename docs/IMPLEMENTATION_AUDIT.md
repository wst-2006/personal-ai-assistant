...
| Local Worker and optional Feishu | Architecture, Roadmap | Partially implemented | High for guarded queue, five-minute recovery, persisted text-candidate and confirmation paths; medium for adapter/webhook tests; low for live delivery | Exact-task writes transactionally create/update/cancel revision-bound reminder jobs. The local worker claims atomically, rejects stale tasks, retries failures, finalizes unanswered focus reminders with a system task outcome, obtains Feishu tenant tokens and sends interactive cards when configured. The official local Feishu long connection now receives owner text, deduplicates by source message ID, persists a parsed candidate, and sends a confirmation card. Only a confirmed exact/daypart task creates a task; ideas/questions create `inbox_entries`; incomplete or conflicting candidates require the desktop app. | `reminder_jobs`, `focus_timer_jobs`, `feishu_intake_candidates`; `reminder-scheduler.ts`, `focus-no-response.ts`, `feishu-intake-service.ts`, `feishu-long-connection.ts`, `apps/worker` | migration guard/schema verification; Worker/provider/webhook tests; Feishu intake PostgreSQL tests for explicit confirmation, idempotency, idea isolation and conflicts; long-connection tests; real browser no-response E2E and builds | In Feishu Open Platform, subscribe the local app to `im.message.receive_v1`, then perform a live owner-only acceptance. Cloud deployment is out of scope. |
...
| Review | Opens/restores a daily review session, saves review messages, reads real task/focus/outcome/feedback context, shows separate same-day software conversation context, generates/edits/confirms/regenerates/exports a brief, and keeps confirmed brief edits persisted through refresh | Richer guided prompts | Partially implemented |
