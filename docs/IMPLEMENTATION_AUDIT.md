...
| AI candidate confirmation and ordinary conversation | Product Spec, Design Review | Partially implemented | High for local persistence, explicit confirmation, provider/conflict/stale recovery, plan-change candidates and live ordinary conversation; medium for provider-dependent output | The AI drawer keeps ordinary user/assistant messages in one local daily conversation, restores them after refresh, and exposes them separately in review context. A separate explicit action parses an editable task, idea, or question candidate. Provider failure leaves the original sentence editable, shows an inline no-write explanation and a clear retry action; only a successful retry exposes the confirmation form. Exact candidate conflicts use an in-app dialog listing the conflicting tasks and times. Returning to adjust preserves every candidate field without a false failure banner or database write; explicit retention alone sends the conflict fingerprint. “另有安排” now receives affected-task lifecycle/schedule context plus optional structured schedule candidates. The server discards unknown, locked and no-op adjustments; each remaining candidate is bound to `version` and `scheduleRevision`, opens the normal editable task form, and writes only after the user's final save. A task changed after consultation rejects the stale candidate. The Planning workspace separately persists editable framework-level task-tree candidates. None of these paths grants AI write authority. | `GET/POST /conversations`, `POST /ai/tasks/parse`, `POST /ai/plan-change-advisories`, versioned task/conflict APIs, task-tree candidate routes; `app_conversations`, `app_conversation_messages`, `tasks`, `inbox_entries`, `long_range_plan_task_tree_candidates` | advisor schema/filter/API tests; conversation service/responder/review tests; real PostgreSQL/browser E2E covers failed parse, preserved input, retry, conflict return-without-write, plan-change no-write candidate review, explicit save, stale rejection, 390px layout, refresh and database persistence | Multi-task all-or-nothing batch application is intentionally absent; current candidates are confirmed and saved individually. Add an atomic batch API only if the user later chooses that different interaction. |
...
| Daily brief | Product Spec, Roadmap | Partially implemented | High for local persistence, structured DeepSeek generation, search-source integrity, edit/confirm/export and desktop/390px paths; medium for device-location coverage | Review generation sends bounded review/task/search material to a server-only DeepSeek structured writer. The validated result contains finance, AI, technology, task-related expansion, humanities/society and encouragement sections; weather/location remains a separate provider snapshot. Search URLs/provider metadata are retained and displayed as links. Search now distinguishes empty reliable results from provider failure, does not cache transient failures, and refuses to persist a false source-backed brief when every search call is unavailable (`503 brief_sources_unavailable`). Malformed model output is retried within the configured budget; final failure returns `503 brief_generation_unavailable` without persistence. A normal conversation still uses a separate explicit AI-drawer action to create a confirmed standalone brief with `reviewSessionId = null`; it is listed, restored after refresh and exportable, and never qualifies for a cyber diary. Confirmed review content remains read-only until explicit edit mode; title, section titles/bodies and summaries persist through the brief API. | `brief-providers.ts`, `brief-service.ts`, `brief-routes.ts`, `ai/brief-writer.ts`, `App.tsx`, `ReviewWorkspace.tsx`; `daily_briefs.content`, `daily_briefs.sources`, nullable `daily_briefs.review_session_id` | writer/provider/domain tests, guarded PostgreSQL no-write source-failure test, API 503 route tests, live review and standalone PostgreSQL/browser E2E (including 390px), mocked browser source-failure recovery and build checks | Add opted-in device location; keep standalone briefs excluded from diary prerequisites. |
...
| Search, weather, location, export | Product Spec, Roadmap | Partially implemented | High for Tavily-backed local brief generation, provider-failure recovery and export; medium for user-entered location | Tavily is the preferred optional LLM-oriented provider, Brave is the secondary optional provider, and free GDELT remains the no-key fallback. Search results carry explicit `ok`, `empty`, or `unavailable` status; unavailable calls are not cached and cannot be mislabeled as “not configured”. If every search section loses its source, brief generation stops before AI/database writes. Open-Meteo geocoding/weather is server-only; an unresolved or temporarily unavailable city remains an explicit weather-section message rather than guessed data. Location is entered per day, and source/snapshot metadata is retained. | `brief-providers.ts`, `brief-service.ts`, `ReviewWorkspace.tsx`, `App.tsx`; `daily_briefs.content`, `daily_briefs.sources` | provider retry/status tests, guarded no-write integration test, 503 route tests, live Tavily standalone PostgreSQL/browser E2E, mocked browser failure recovery and review browser E2E | Add user-authorized device location; current manual-city path remains valid and provider failures are explicit. |
...
| Global backup control | A user-triggered download requests a versioned, read-only logical JSON backup from one PostgreSQL snapshot. Format v5 covers every current application table, including long-range planning/task-tree and user-profile data; no page state, environment values, credentials, runtime files, or PostgreSQL directory contents are included. | Restore/import is intentionally absent because it requires a separately validated destructive-operation design | Fully implemented for export |
