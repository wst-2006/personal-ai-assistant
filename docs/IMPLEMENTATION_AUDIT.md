...
| Weekly health reference | Latest health-reference baseline | Partially implemented | High for local profile, versioned candidate/confirmation, PostgreSQL refresh and browser path; medium for upload boundary and structured persistence; low for provider-dependent visual extraction | Separate Health page initializes a user-controlled local health profile, presents a seven-day read-only food/movement-range candidate, lets the user optionally request an explicit DeepSeek refinement, and requires confirmation before a candidate becomes the active week. City is intentionally optional and never inferred. User-selected PNG/JPEG/WebP sleep screenshots are validated, sent transiently through a replaceable vision adapter, and persisted only as a hash plus structured visible metrics; no original image is stored and no plan is changed. | `health-routes.ts`, `health-service.ts`, `health-planner.ts`, `sleep-image-analyzer.ts`, `HealthWorkspace.tsx`; `health_profiles`, `health_week_plans`, `health_daily_references`, `health_sleep_analyses` | domain/schema/guarded PostgreSQL tests, API route tests, browser E2E for candidate/confirmation/refresh/no task writes, upload UX and 390px | Configure and verify a vision-capable provider model; add explicit user-requested sleep-based plan-revision candidates only after the separate confirmation workflow is designed. |
...
| Health | User-controlled profile, active/candidate weekly references, and structured sleep screenshot analyses are API-backed and restored from PostgreSQL. The default local candidate gives only ranges, plate guidance, seasonable vegetables, category/duration/intensity and safety prompts; it never creates health tasks or scores. DeepSeek refinement and vision extraction are explicit and remain separate from confirmed plan data. | A vision-capable provider model still needs real credential/model verification; user-requested “adjust plan from this sleep” candidate flow and richer manual weekly editing remain unimplemented; no Today summary card by design in this slice | Partially implemented |
...
8. **Later dependencies remain absent.** Local Feishu controls, PWA,
   cross-device verification, and user-requested sleep-based plan revisions
   still need explicit implementation phases; cloud runtime is deferred and
   the durable local Worker queue now exists.
