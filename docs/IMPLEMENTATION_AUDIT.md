...
| Daily brief | Product Spec, Roadmap | Partially implemented | Medium for review-derived draft, dynamic location and provider boundaries; low for live external acceptance | Review may generate, edit, regenerate and confirm a persisted brief with finance, AI, technology, task-expansion, humanities, weather and location sections. The user enters that day's location in the review page; geocoded location and weather snapshots persist in brief JSON. | `brief-providers.ts`, brief routes, `ReviewWorkspace.tsx`; `daily_briefs.content`, `daily_briefs.sources` | domain/provider tests, 390px browser E2E and build checks | Add opted-in device location, live-provider acceptance, export and normal-chat path. |
