...
| Weekly health reference | Latest health-reference baseline | Partially implemented | High for local profile, versioned candidate/confirmation, PostgreSQL refresh and browser path; low for screenshot analysis | Separate Health page initializes a user-controlled local health profile, presents a seven-day read-only food/movement-range candidate, lets the user optionally request an explicit DeepSeek refinement, and requires confirmation before a candidate becomes the active week. City is intentionally optional and never inferred. | `health-routes.ts`, `health-service.ts`, `health-planner.ts`, `HealthWorkspace.tsx`; `health_profiles`, `health_week_plans`, `health_daily_references` | domain/schema/guarded PostgreSQL tests plus browser E2E for candidate, explicit confirmation, refresh, no task writes and 390px | Add user-uploaded sleep-image analysis as a separate safe file/vision-adapter slice; add richer manual weekly editing and an optional compact Today summary without converting recommendations into tasks. |
...
| Health | User-controlled profile plus active/candidate weekly references are API-backed and restored after refresh. The default local candidate gives only ranges, plate guidance, seasonable vegetables, category/duration/intensity and safety prompts; it never creates health tasks or scores. DeepSeek refinement is explicit and remains a candidate until confirmation. | Sleep screenshot import/analysis and richer manual weekly editing are not yet implemented; no Today summary card by design in this slice | Partially implemented |
